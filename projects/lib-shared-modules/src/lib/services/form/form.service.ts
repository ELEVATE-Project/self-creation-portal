...
export class FormService {
  ...
  getEntitiesListAsType(url: string,entityType?:string,id?:string,page?:number,itemsPerPage?:number,sortKey?:string,sortOrder?:string) {
    return this.httpService.get(url+ (id ? ("/"+id) :'')+ (entityType ? '?'+`type=${entityType}` : '') + (page ? '&'+`page=${page}` : '') + (itemsPerPage ? '&'+`limit=${itemsPerPage}`+`&parentInfoRequired=true&` : '')+ (sortKey ?`sortKey=${sortKey}&` : ''  )+(sortKey ?`sortOrder=${sortOrder}` : ''  ));
  }
